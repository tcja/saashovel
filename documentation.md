...
Create pages through the Filament admin panel's CMS.

This method provides a user-friendly interface with a built-in permission system to restrict page access based on subscription tiers.

**Tip:** The CMS method is recommended for content-focused pages where you need simple subscription-based access control.

**Note:** The main page template for pages created via the CMS can be customized here:
...
```php
// This route is only accessible to users with a subscription.
...
Use Laravel's standard asset management for static JavaScript files.

- Location: `resources/js/`
- Compile using Vite or your preferred bundler.
- Import scripts in `resources/js/app.js`.

**Best practice:** Use traditional asset management for static JavaScript files that do not require dynamic Blade variables.

#### 2. Dynamic Assets (`merged-assets`)

Use dynamic assets for JavaScript that requires Blade variables or dynamic content.

- Assets directory: `resources/views/livewire/pages/assets/`
- Default assets file: `resources/views/livewire/pages/assets/default-merged-assets.blade.php`
- Custom assets file: `resources/views/livewire/pages/assets/merged-assets.blade.php`

**Important:** The `default-merged-assets.blade.php` file contains essential code for subscription management. Do not modify it. Add your own dynamic assets inside `merged-assets.blade.php`.

#### Implementation

The merged assets files are included in both the app and guest layouts:
...
Use `merged-assets.blade.php` for:

- Scripts requiring Blade variables
- Dynamic route generation
- Payment provider integration
- SPA functionality
- Other dynamic scripts
...
2. Include it in `merged-assets.blade.php`.
...
This component checks the user's authentication status and loads the appropriate view.
...
Upon registration, users are automatically assigned the `user` role with basic feature access.

When subscribing to a plan, their role is upgraded to the corresponding tier role.

Users can only have one role at a time, though each role may grant multiple permissions.
...
Use the available permission options from the page editor to restrict CMS page access based on roles or subscription tiers.
...
| `premium-second-tier` | `access basic features`, `premium features`, `first tier features`, `second tier features` |
...
- You can modify `database/seeders/AdminUserSeeder.php` to customize default users and roles.
...
SaaShovel includes a flexible theming system that allows you to customize the entire look of your application.

Themes are configured in `config/themes.php` and can be switched using the `APP_THEME` environment variable.
...
Component-specific styles include:
...
Laravel and Livewire automatically handle CSRF protection.

Every form should include the directive:
...
For raw HTML, use this syntax with caution:
...
Protect sensitive properties using the `#[Locked]` attribute:

```php
#[Locked]
public $sensitiveData; // Cannot be modified from the frontend.
```

Use query string configuration carefully:
...
- Use prepared statements. Laravel's query builder handles this by default.
- Keep dependencies updated with `composer audit`.
- Set proper file permissions, for example `storage: 775` and `config: 644`.
...
Use middleware groups to protect routes:
...
The application verifies webhook signatures for all payment providers:
...
This ensures that only legitimate requests from the payment providers are processed.

Important webhook security practices:

- Keep webhook secrets secure in your `.env` file.
...
Secure file upload handling example:

```php
// Validate file uploads.
'document' => ['required', 'file', 'mimes:pdf,docx', 'max:10240'];

// Store files in a private disk.
Storage::disk('private')->put($path, $file, 'private');
```

Regular security updates and audits are crucial.

Use the following command to check for known vulnerabilities in your dependencies:
